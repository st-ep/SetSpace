...
At test time we resample each object under five density
distributions---\emph{uniform}, \emph{polar} (concentrated near the poles),
\emph{equatorial} (concentrated near the equator), \emph{clustered}
(concentrated around two random centers), and \emph{hemisphere} (visible from
a random viewpoint)---at each of five point counts
...
sampled surface cells per object and evaluated under six sampling modes:
\emph{uniform}, \emph{front}, \emph{rear}, \emph{roof}, \emph{clustered}, and
\emph{occluded}.  The nonuniform modes are obtained by reweighting the base
surface-area distribution, so the sampling pattern changes while the
underlying CFD solution remains fixed.  We evaluate at point counts
...
airfoil boundary points and then evaluated under seven observation modes:
\emph{uniform}, \emph{leading edge}, \emph{trailing edge},
\emph{upper surface}, \emph{lower surface}, \emph{clustered}, and
\emph{occluded}.  These modes reweight the base boundary-length distribution,
so the observed subset shifts around the same underlying airfoil solution
while the reference coefficients remain fixed.  We test at point counts

...
that differ only in the pooling weights: \textbf{Set-Key (Unif)} with standard
additive pooling and \textbf{Set-Key (kNN)} with geometry-aware weighting
($k{=}8$).  On the synthetic sphere benchmark, where the target measure is
known exactly, we additionally report \textbf{Set-Value (Vor)}, which uses exact
spherical Voronoi cell areas under the uniform surface measure and replaces
the learned Set-Key aggregation with a normalized Voronoi-weighted average of
the observed scalar values.  All learned models are trained on uniformly
...
The learned Set-Key regressors share the same weighted set encoder
architecture: 16 learnable query tokens, key dimension 64, token dimension 32,
hidden dimension 128, and a three-layer MLP head.  They differ only in their
aggregation weights---$1/N$, $k$-NN density estimates ($k{=}8$, intrinsic
dimension~$d{=}2$), or exact spherical Voronoi cell areas under uniform
surface measure.  To test how much of this task can be solved by geometry
alone, we additionally report the parameter-free Set-Value (Vor) baseline, which
directly computes the normalized Voronoi-weighted average of the observed scalar
values.
The learned Set-Key models are trained for 2\,000 steps on
batches of 64 objects, each observed through 128 uniformly sampled points,
using AdamW (learning rate~$10^{-3}$, weight decay~$10^{-5}$) with gradient
clipping at norm~1.
...
\text{Set-Key (kNN)} lowers in-distribution RMSE from $0.046$ to $0.021$,
lowers worst-case RMSE under sampling shift from $0.248$ to $0.188$, and
reduces average nonuniform prediction drift from $0.159$ to $0.087$.  With
exact Voronoi weights, \text{Set-Value (Vor)} reaches $0.012$, $0.137$, and
$0.059$.  This makes the benchmark interpretation especially clear: once
exact geometric weights are available, the task is dominated by quadrature
quality.  The same trend is visible under refinement in
Figure~\ref{fig:regression-convergence}: \text{Set-Value (Vor)} follows the
lowest curve, \text{Set-Key (kNN)} remains clearly below
\text{Set-Key (Unif)}, and PointNeXt serves as a larger-backbone reference
rather than an
explicitly weighted quadrature model.
...
$N{=}512$, \text{Set-Value (Vor)} and \text{Set-Key (kNN)} both produce
noticeably tighter true-versus-predicted scatter and smaller residual clouds
than \text{Set-Key (Unif)}, while the PointNeXt
...
\begin{figure*}[ht]
  ...
  \caption{Synthetic mean-regression refinement curves. Average nonuniform RMSE (left), shown here as a smoother refinement diagnostic complementary to the worst-case table metric, and average nonuniform prediction drift (right), defined as the mean absolute change from the dense-reference prediction of the same object, as the number of observed sphere samples increases. The synthetic-only Set-Value (Vor) baseline uses exact spherical cell areas, making this benchmark a direct quadrature test once exact geometric weights are available. Dotted lines show log-log power-law fits.}
...
\input{tables/table_sampling_breakdown}
\input{tables/table_resolution_refinement}
...
\begin{figure*}[ht]
  ...
  \caption{AhmedML predictions under front-biased sampling at $N{=}1024$. A representative test body with observed cells overlaid (left), and true-vs-predicted scatter for $C_d$ (middle) and $C_l$ (right) comparing Set-Key (Unif), Set-Key (kNN), and PointNeXt.}
...
\input{tables/table_airfrans_sampling_breakdown}
\input{tables/table_airfrans_resolution_refinement}
...
\begin{figure*}[ht]
  ...
  \caption{AirfRANS predictions under trailing-edge-biased sampling at $N{=}1024$. Left: a representative test airfoil colored by boundary pressure, with the observed boundary samples overlaid. Middle and right: true-versus-predicted scatter plots for $C_d$ and $C_l$ across the full AirfRANS test split, comparing Set-Key (Unif), Set-Key (kNN), and PointNeXt under the same observation pattern. The diagonal line indicates perfect prediction.}

...
def _sample_uniform_sphere(n_points: int, generator: torch.Generator) -> torch.Tensor:
    return _normalize(torch.randn((n_points, 3), generator=generator, dtype=torch.float32))
...
def sample_surface_points(
    ...
    candidate_count = max(n_points * int(candidate_multiplier), 2048)
    ...
    sample_idx = torch.multinomial(scores, n_points, replacement=False, generator=generator)

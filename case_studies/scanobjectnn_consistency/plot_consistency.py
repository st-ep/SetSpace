...
def plot_metrics(metrics: dict, output_dir: Path, fixed_points: int | None = None) -> None:
    ...
    width = 0.36
    uniform_scores = [
        metrics["models"]["uniform"]["metrics"]["aggregate"]["accuracy_by_count"][fixed_key][mode]
        for mode in sampling_modes
    ]
    geom_scores = [
        metrics["models"]["geometry_aware"]["metrics"]["aggregate"]["accuracy_by_count"][fixed_key][mode]
        for mode in sampling_modes
    ]
